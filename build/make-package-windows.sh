...
cp -r assets/common/lualibs ${PACKAGE_DIR};
cp $(expand-path ${LUASOCKET_FILES2}) ${PACKAGE_DIR}/lualibs/;
mkdir -p  ${PACKAGE_DIR}/lualibs/socket;
cp $(expand-path ${LUASOCKET_FILES1}) ${PACKAGE_DIR}/lualibs/socket/;
cp ${LUASOCKET_SOCKETDLL} ${PACKAGE_DIR}/lualibs/socket/core.dll;
mkdir -p ${PACKAGE_DIR}/lualibs/mime/;
cp ${LUASOCKET_MIMEDLL} ${PACKAGE_DIR}/lualibs/mime/core.dll;
cp $(expand-path ${LUACJSON_FILES}) ${PACKAGE_DIR}/lualibs/;
cp $(expand-path ${LUAFILESYSTEM_FILES}) ${PACKAGE_DIR}/lualibs/;
cp $(expand-path ${LUAXML_FILES}) ${PACKAGE_DIR}/lualibs/;
cp $(expand-path ${LUAWINALTTAB_FILES}) ${PACKAGE_DIR}/lualibs/;
rm -f ${PACKAGE_DIR}/lualibs/__*;
