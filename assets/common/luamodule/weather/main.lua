...
local script_path = ibs.join_path(ibs.CONFIG_DIR, "plugins", "weather")
...
system.completer.option_func["weather"] = function(values)
  local xml_file = ibs.join_path(script_path, "data.xml")
  local xml_text = ""
  ...
  for i, item in ipairs(xo(xml_object, "rss", "channel")) do
    if i > 13 and item[0] == "item" then
      ...
      local iconpath = ibs.join_path(script_path, iconfile)
...
commands["weather"] = { 
  ...
  icon=ibs.join_path(script_path, "weather.png"), 
  history=false
}
