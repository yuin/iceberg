...
0.9.10 (2016-08-30)
-----------------------
- IMPROVED: 試験的にWindowsでDirectWriteを実装。パフォーマンス上の問題があるためデフォルトはGDIのままとする。

0.9.9からのアップグレード方法
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
- 0.9.10をダウンロードしてください。
- 0.9.10から ``iceberg.exe`` をコピーして既存の0.9.9側に上書きしてください。
- ``config.lua`` の ``system`` グローバル変数に ``disable_direct_draw=true``  を追加してください。DirectDraw機能を利用する場合は ``false`` をセットしてください。(ただし、現段階では描画が遅いです)
